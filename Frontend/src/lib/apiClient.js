...
let baseGetter = () => (process.env.REACT_APP_API_BASE || "");

/**
 * PUBLIC_INTERFACE
 * configureApiBaseGetter
 * Set a function that returns the current API base URL at call time.
 * This enables runtime updates (e.g., via context).
 */
export function configureApiBaseGetter(getterFn) {
...
async function authedFetch(getSessionFn, path, options = {}) {
  const base = (baseGetter && baseGetter()) || "";
  const url = `${base}${path}`;
